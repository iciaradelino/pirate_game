...
void remove_item_from_room(Room *room, ItemID item_id)
{
    ...
    if (found_idx != -1)
    {
        for (i = found_idx; i < room->item_count - 1; ++i)
        {
...
void init_rooms(GameState *gs)
{
    gs->rooms[ROOM_DECK] = (Room)
    {
        ...
        { NUM_ROOMS, NUM_ROOMS, ROOM_GALLEY, NUM_ROOMS },
        { ITEM_NONE },
        0,
        0
    };
    ...
    gs->rooms[ROOM_CAPTAIN_QUARTERS] = (Room)
    {
    ...
    gs->rooms[ROOM_PRISON_HOLD] = (Room)
    {
        ROOM_PRISON_HOLD,
        "Prison Hold",
    ...
    gs->rooms[ROOM_TREASURE_ROOM] = (Room)
    {
        ROOM_TREASURE_ROOM,
        "Treasure Room",
